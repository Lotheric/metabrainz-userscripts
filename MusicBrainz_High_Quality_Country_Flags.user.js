...
(function () {
  ...
  function ensureFlagMap() {
    if (flagDataMap.size === 0) {
      COUNTRIES.forEach(c => {
        ...
        uuidToCodeMap.set(c.uuid.toLowerCase(), c.code);
      });
    }
  }
  ...
  function processFlags() {
    try {
      ...
      // Process unflagged area links (e.g. search results)
      ...
      document.querySelectorAll('a[href*="/area/"]:not([data-hq-processed]):not([data-hq-skip])').forEach(link => {
        ...
        // If the link is a native MB prepended annotation icon, or already in a .flag wrapper, skip it
        if (!link.textContent.trim() || link.closest('.area-icon') || link.closest('.type-icon') || link.querySelector('img') || link.closest('.flag')) {
          return;
        }
